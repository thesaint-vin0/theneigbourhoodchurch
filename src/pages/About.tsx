...
const valueIcons: Record<string, React.ComponentType<{ className?: string }>> = {
  heart: Heart, book: Book, hand: Hand, users: Users, gift: Gift, star: Star,
};
...
function StatCounter({ value, suffix, label }: { value: number; suffix: string; label: string }) {
  ...
  return (
    <div ref={ref} className="text-center">
      <div className="font-display text-4xl md:text-5xl font-700 text-primary-800">{count.toLocaleString()}{suffix}</div>
...
export default function About() {
  return (
    <>
      <PageHeader
        title="About Us"
        subtitle="Where Faith Meets Family — a community where everyone belongs."
        image="https://images.pexels.com/photos/4330187/pexels-photo-4330187.jpeg?auto=compress&cs=tinysrgb&w=1200"
      />
      ...
      <section className="pb-20 px-6 md:px-12">
        <div className="container-tnc grid md:grid-cols-2 gap-6">
          <Reveal>
      ...
      <section className="px-6 md:px-12 pb-20">
        <div className="container-tnc">
          <Reveal>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8 py-10 px-6 glass rounded-3xl">
            ...
            {coreValues.map((v, i) => {
              ...
              return (
                <Reveal key={v.title} delay={i * 0.08}>
                  <div className="group p-7 rounded-2xl border border-gray-100 bg-white hover:shadow-glow transition-all duration-500">
